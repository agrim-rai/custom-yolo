...
class C3k2(nn.Module):
    """CSP-style block (replaces C2f) with same I/O. Uses smaller effective receptive field via (1,3)+(3,1) cross-convs (k=2 style)."""
    ...
    def __init__(
        ...
        # cross-conv (1,3)+(3,1) preserves size like 3x3 but lighter; same interface as C2f
        self.m = nn.ModuleList(
            Bottleneck(self.c, self.c, shortcut, g, k=((1, 3), (3, 1)), e=1.0) for _ in range(n)
        )
...
class C2PSA(nn.Module):
    """Lightweight partial self-attention: split channels, attention on one branch, conv on other, merge and project."""
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, e: float = 0.5):
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv(2 * self.c, c1, 1)
        # lightweight self-attention branch (depthwise conv as cheap spatial mixing)
        self.attn = nn.Sequential(
            DWConv(self.c, self.c, 3, 1),
            Conv(self.c, self.c, 1, 1),
        )
        self.conv_branch = Conv(self.c, self.c, 3, 1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        a, b = self.cv1(x).split((self.c, self.c), dim=1)
        b = b + self.attn(b)
        b = b + self.conv_branch(b)
        return self.cv2(torch.cat((a, b), 1))
